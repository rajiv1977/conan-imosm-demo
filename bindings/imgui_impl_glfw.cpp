...
//  2023-01-04: Inputs: Fixed mods state on Linux when using Alt-GR text input (e.g. German keyboard layout), could lead
//  to broken text input. Revert a 2022/01/17 change were we resumed using mods provided by GLFW, turns out they were
//  faulty. 2022-11-22: Perform a dummy glfwGetError() read to cancel missing names with glfwGetKeyName(). (#5908)
//  2022-10-18: Perform a dummy glfwGetError() read to cancel missing mouse cursors errors. Using GLFW_VERSION_COMBINED
//  directly. (#5785) 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11. 2022-09-26: Inputs:
//  Renamed ImGuiKey_ModXXX introduced in 1.87 to ImGuiMod_XXX (old names still supported). 2022-09-01: Inputs: Honor
//  GLFW_CURSOR_DISABLED by not setting mouse position. 2022-04-30: Inputs: Fixed
//  ImGui_ImplGlfw_TranslateUntranslatedKey() for lower case letters on OSX. 2022-03-23: Inputs: Fixed a regression
//  in 1.87 which resulted in keyboard modifiers events being reported incorrectly on Linux/X11. 2022-02-07: Added
//  ImGui_ImplGlfw_InstallCallbacks()/ImGui_ImplGlfw_RestoreCallbacks() helpers to facilitate user installing callbacks
//  after initializing backend. 2022-01-26: Inputs: replaced short-lived io.AddKeyModsEvent() (added two weeks ago) with
//  io.AddKeyEvent() using ImGuiKey_ModXXX flags. Sorry for the confusion. 2021-01-20: Inputs: calling new
//  io.AddKeyAnalogEvent() for gamepad support, instead of writing directly to io.NavInputs[]. 2022-01-17: Inputs:
//  calling new io.AddMousePosEvent(), io.AddMouseButtonEvent(), io.AddMouseWheelEvent() API (1.87+). 2022-01-17:
//  Inputs: always update key mods next and before key event (not in NewFrame) to fix input queue with very low
//  framerates. 2022-01-12: *BREAKING CHANGE*: Now using glfwSetCursorPosCallback(). If you called
