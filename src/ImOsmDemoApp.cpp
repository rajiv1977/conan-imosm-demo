...
ImOsmDemoApp::ImOsmDemoApp()
    ...
    , _markEditorWidget{std::make_unique<MarkEditorWidget>(_mapPlot, _markStorage)}
    , _distanceCalcWidget{std::make_unique<DistanceCalcWidget>(_markStorage)}
    , _destinationCalcWidget{std::make_unique<DestinationCalcWidget>(_markStorage)}
...
void ImOsmDemoApp::paint()
{
    ...
    if (_worldBtn.handle())
    {
        _mapPlot->setBoundsGeo(ImOsm::MIN_LAT, ImOsm::MAX_LAT, ImOsm::MIN_LON, ImOsm::MAX_LON);
    }

    ImGui::SameLine();
    _trontoBtn.paint();
    if (_trontoBtn.handle())
    {
        _mapPlot->setBoundsGeo(43.625992, 43.700997, -79.471815, -79.278738);
    }
