...
void TileSourceWidget::paint()
{
    ...
    ImGui::SameLine();
    ImGui::InputText("Path / URL", &_ui->source[0], sizeof(_ui->source) / sizeof(_ui->source[0]));
    ...
    if (ImGui::Button("OSM"))
    {
        ...
        _mapPlot->setTileLoader(std::make_shared<TileLoaderOsmMap>(_ui->requestLimit));
    };
    ImGui::SameLine();
    if (ImGui::Button("ARC"))
    {
        _ui->source = TileSourceUrlArc::URL_TPL;
        _mapPlot->setTileLoader(std::make_shared<TileLoaderArcMap>(_ui->requestLimit));
    };
...
void TileSourceWidget::updateTileLoader()
{
    ...
    {
        _tileLoader = std::make_shared<TileLoaderUrlMap>(_ui->source, _ui->requestLimit);
    }
    else if (!_ui->source.empty())
    {
        _tileLoader = std::make_shared<TileLoaderFsMap>(_ui->source, _ui->requestLimit);
    }
