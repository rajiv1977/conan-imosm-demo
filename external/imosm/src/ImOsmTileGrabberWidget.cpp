...
void ImOsm::TileGrabberWidget::paint()
{
    ...
    ImGui::Text("Min Lat %.6f, Min Lon %.6f", _mapPlot->minLat(), _mapPlot->minLon());
    ImGui::Text("Max Lat %.6f, Max Lon %.6f", _mapPlot->maxLat(), _mapPlot->maxLon());
    ...
    ImGui::Text(
        "Tiles Count: %lu",
        countTiles(
            _mapPlot->minLat(), _mapPlot->maxLat(), _mapPlot->minLon(), _mapPlot->maxLon(), _ui->minZ, _ui->maxZ));

    ImGui::InputText("Tile Server URL##", &_ui->url[0], sizeof(_ui->url) / sizeof(_ui->url[0]));
    ...
    ImGui::InputText("Destination Dir##", &_ui->dirname[0], sizeof(_ui->dirname) / sizeof(_ui->dirname[0]));
    if (ImGui::Button("Grab"))
    {
        _ui->tileTotal = countTiles(
            _mapPlot->minLat(), _mapPlot->maxLat(), _mapPlot->minLon(), _mapPlot->maxLon(), _ui->minZ, _ui->maxZ);
        _tileGrabber = std::make_unique<TileGrabber>(
            std::make_shared<TileSourceUrlCustom>(_ui->requestLimit, _ui->preload, _ui->url),
