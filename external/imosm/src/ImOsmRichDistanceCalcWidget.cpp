...
DistanceCalcWidget::DistanceCalcWidget(std::shared_ptr<MarkStorage> storage)
    : _storage{storage}
    , _ui{std::make_unique<DistanceCalcWidget::Ui>()}
{
}
...
void DistanceCalcWidget::paint()
{
    ...
    ImGui::InputText("Mark A##", &_ui->markNameA[0], sizeof(_ui->markNameA) / sizeof(_ui->markNameA[0]));
    ImGui::SameLine();
    ImGui::SetNextItemWidth(100.f);
    ImGui::InputText("Mark B##", &_ui->markNameB[0], sizeof(_ui->markNameB) / sizeof(_ui->markNameB[0]));
    ImGui::SameLine();
    ...
    ImGui::Text("Midpoint [deg] Lat: %.6f Lon: %.6f", _ui->markC.lat, _ui->markC.lon);
    ImGui::SameLine();
