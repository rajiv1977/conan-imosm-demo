...
struct MapPlot::Impl
{
    ...
    constexpr static const ImPlotAxisFlags xFlags{ImPlotAxisFlags_NoLabel | ImPlotAxisFlags_NoGridLines |
...
void ImOsm::MapPlot::setBoundsGeo(float minLat, float maxLat, float minLon, float maxLon)
{
    _minLat    = minLat;
    _maxLat    = maxLat;
    _minLon    = minLon;
...
void ImOsm::MapPlot::getBoundsGeo(float& minLat, float& maxLat, float& minLon, float& maxLon) const
{
    minLat = _minLat;
    maxLat = _maxLat;
    minLon = _minLon;
...
void ImOsm::MapPlot::getBoundsTile(int& minTX, int& maxTX, int& minTY, int& maxTY) const
{
    minTX = _minTX;
    maxTX = _maxTX;
    minTY = _minTY;
...
void ImOsm::MapPlot::setBoundsLocal(float minX, float maxX, float minY, float maxY)
{
    _minX      = minX;
    _maxX      = maxX;
    _minY      = minY;
...
void ImOsm::MapPlot::getBoundsLocal(float& minX, float& maxX, float& minY, float& maxY) const
{
    minX = _minX;
    maxX = _maxX;
    minY = _minY;
...
{
    return _impl->mousePos.x > _impl->plotLims.X.Min && _impl->mousePos.x < _impl->plotLims.X.Max &&
           _impl->mousePos.y > _impl->plotLims.Y.Min && _impl->mousePos.y < _impl->plotLims.Y.Max;
}
