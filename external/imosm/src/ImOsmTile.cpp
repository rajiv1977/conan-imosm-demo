...
namespace ImOsm
{
ImOsm::Tile::Tile(int z, int x, int y, const std::vector<std::byte>& rawBlob, bool preload)
    : TileDummy(z, x, y)
    , _rawBlob{rawBlob}
{
    if (preload)
...
unsigned int Tile::rawBlobSize() const
{
...
unsigned int Tile::rgbaBlobSize() const
{
...
void Tile::stbLoad() const
{
    ...
    const auto ptr{stbi_load_from_memory(reinterpret_cast<stbi_uc const*>(_rawBlob.data()),
                                         static_cast<int>(_rawBlob.size()),
                                         &_pxW,
                                         &_pxH,
                                         &_channels,
                                         STBI_rgb_alpha)};
...
void Tile::glLoad() const
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, _pxW, _pxH, 0, GL_RGBA, GL_UNSIGNED_BYTE, rgbaBlob());
}
}; // namespace ImOsm
