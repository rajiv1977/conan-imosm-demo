...
void TileLoaderUrl::beginLoad(int z, int xmin, int xmax, int ymin, int ymax)
{
    ...
    _tiles.erase(std::remove_if(_tiles.begin(), _tiles.end(), rm_cond), _tiles.end());
...
{
    auto it{std::find(_tiles.begin(), _tiles.end(), Tile{std::array<int, 3>{z, x, y}})};
    ...
    if (it == _tiles.end())
    {
        if (_futureCounter++ < _futureLimit)
        {
            _tiles.insert(
                it,
                {{z, x, y},
                 std::async(std::launch::async, &TileLoaderUrl::onHandleRequest, this, std::array<int, 3>({z, x, y}))});
        }
...
TileLoaderUrl::Tile::Remote TileLoaderUrl::onHandleRequest(const std::array<int, 3>& zxy)
{
...
size_t TileLoaderUrl::onPullResponse(void* data, size_t size, size_t nmemb, void* userp)
{
