...
MarkItemWidget::MarkItemWidget(std::shared_ptr<MarkItem> item, const GeoCoords& latLonPicked)
    : _item{item}
    ...
    , _ui{std::make_unique<Ui>()}
{
    ...
    _ui->markerFill     = {_item->style().markerFill.x, _item->style().markerFill.y, _item->style().markerFill.z};
}
...
{
    ImGui::InputText("Name", &_ui->text[0], sizeof(_ui->text) / sizeof(_ui->text[0]));
...
void MarkItemWidget::apply()
{
    ...
    _item->style().markerFill   = {_ui->markerFill[0], _ui->markerFill[1], _ui->markerFill[2], 1.0};
}
...
void MarkItemWidget::paint_markerCombo()
{
    ...
    {
        if (ImGui::Selectable(MarkerTypeName(ImPlotMarker_Circle), _ui->markerType == ImPlotMarker_Circle))
        {
            _ui->markerType     = ImPlotMarker_Circle;
            _ui->markerTypeName = MarkerTypeName(ImPlotMarker_Circle);
        }
        else if (ImGui::Selectable(MarkerTypeName(ImPlotMarker_Square), _ui->markerType == ImPlotMarker_Square))
        {
            _ui->markerType     = ImPlotMarker_Square;
            _ui->markerTypeName = MarkerTypeName(ImPlotMarker_Square);
        }
        else if (ImGui::Selectable(MarkerTypeName(ImPlotMarker_Diamond), _ui->markerType == ImPlotMarker_Diamond))
        {
            _ui->markerType     = ImPlotMarker_Diamond;
            _ui->markerTypeName = MarkerTypeName(ImPlotMarker_Diamond);
        }
        else if (ImGui::Selectable(MarkerTypeName(ImPlotMarker_Up), _ui->markerType == ImPlotMarker_Up))
        {
