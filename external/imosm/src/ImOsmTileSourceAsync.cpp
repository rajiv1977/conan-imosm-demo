...
{
    const auto it{std::find(_requests.begin(), _requests.end(), TileAsync{z, x, y})};
...
bool TileSourceAsync::request(int z, int x, int y)
{
    ...
    {
        _requests.push_back(
            TileAsync{z, x, y, std::async(std::launch::async, &TileSourceAsync::onHandleRequest, this, z, x, y)});
        return true;
    }
    return false;
}
...
{
    return std::all_of(std::begin(_requests), std::end(_requests), [](TileAsync& tile) { return tile.ready(); });
}
