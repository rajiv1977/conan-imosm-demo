...
bool MarkItem::inBounds(float minLat, float maxLat, float minLon, float maxLon) const
{
    /*
...
void MarkItem::paint()
{
    if (_style.markerEnabled)
    {
        ImPlot::SetNextMarkerStyle(
            _style.markerType, _style.markerSize, _style.markerFill, _style.markerWeight, _style.markerOutline);
    ...
    if (_style.textEnabled)
    {
        ...
        ImPlot::PlotText(_text.c_str(), _osmCoords.x, _osmCoords.y, {0.f, _style.markerSize + ImGui::GetFontSize()});
        ImGui::PopID();
        ImGui::PopStyleColor();
    }
...
void MarkItem::updateRadiusPoints()
{
    ...
    const double r{sqrt(pow(x - _osmCoords.x, 2.0) + pow(y - _osmCoords.y, 2.0))};
