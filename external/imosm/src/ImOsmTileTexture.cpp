...
TileTexture::TileTexture(int size, const std::vector<std::byte>& blob)
{
    ...
    const auto ptr{stbi_load_from_memory(reinterpret_cast<stbi_uc const*>(blob.data()),
    ...
    if (ptr)
    {
        ...
        _blob.insert(_blob.begin(), byteptr, byteptr + _width * _height * STBI_rgb_alpha);
        stbi_image_free(ptr);
    }
}
...
void TileTexture::loadTexture() const
{
    ...
    glTexImage2D(GL_TEXTURE_2D,
                 ...
                 GL_RGBA,
                 GL_UNSIGNED_BYTE,
                 reinterpret_cast<const char*>(_blob.data()));
}
} // namespace Old
} // namespace ImOsm
