...
MarkEditorWidget::MarkEditorWidget(std::shared_ptr<RichMapPlot> plot, std::shared_ptr<MarkStorage> storage)
    : _plot{plot}
    , _storage{storage}
...
void MarkEditorWidget::paint()
{
    ...
    if (_storage->handleLoadState())
    {
        ...
        std::for_each(markItems.begin(), markItems.end(), [this](auto& item) { _plot->addItem(item.ptr); });
    }
...
void MarkEditorWidget::paint_markNameInput()
{
    ...
    ImGui::InputText(
        "Mark Name", &_ui->markNameInputText[0], sizeof(_ui->markNameInputText) / sizeof(_ui->markNameInputText[0]));
    ImGui::PopItemWidth();
}
...
void MarkEditorWidget::paint_markTableRow(const MarkStorage::ItemNode& item)
{
    ...
    if (ImGui::Button("Setup"))
    {
        ...
        _itemWidget = std::make_unique<MarkItemWidget>(item.ptr, GeoCoords{_ui->latLonInput});
    }
