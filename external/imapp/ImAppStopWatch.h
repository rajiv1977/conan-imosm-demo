...
template <StopWatchType TimerType>
class StopWatchTimer
{
  public:
    ...
    inline void stop()
    {
        ...
        {
            _us = std::chrono::duration_cast<std::chrono::microseconds>(_endTime - _beginTime).count();
        }
        else if constexpr (_timerType == StopWatchType::Max)
        {
            _us = std::max(_us, std::chrono::duration_cast<std::chrono::microseconds>(_endTime - _beginTime).count());
        }
    }
